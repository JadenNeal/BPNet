...
class SEResNet(nn.Module):
    def __init__(self, model_width, layer_dims, ratio=16):
        ...
        self.stem = nn.Sequential(
            # two channel, ppg & ecg
            nn.Conv1d(2, model_width, kernel_size=7, stride=2, padding=3, bias=False),
...
class BPNet(nn.Module):
    def __init__(self, model_width, layer_dims, ratio):
        ...
        self.share_net = SEResNet(model_width, layer_dims, ratio)
    ...
    def forward(self, ppg_x, ecg_x):
        fusion = torch.cat((ppg_x, ecg_x), dim=1)
