...
import os
from scipy.stats import boxcox, gmean
...
def compute_metrics(y_hat, y_true, y_train, save_path, configs):
    ...
    sbp_shot_dict = shot_metrics(y_hat[:, 0], y_true[:, 0], y_train[:, 0], many_shot=configs["sbp_many"], low_shot=configs["sbp_low"])
    dbp_shot_dict = shot_metrics(y_hat[:, 1], y_true[:, 1], y_train[:, 1], many_shot=configs["dbp_many"], low_shot=configs["dbp_low"])
...
def shot_metrics(y_hat, y_true, y_train, many_shot=6000, low_shot=1000):
    ...
    # print("many shot cnt: ", np.sum(many_shot_cnt))
    # print("median shot cnt: ", np.sum(median_shot_cnt))
    # print("low shot cnt: ", np.sum(low_shot_cnt))
...
def _prepare_data(path):
    X = np.load(os.path.join(path, "all_wav.npy"))
    Y = np.load(os.path.join(path, "all_bp.npy"))

    ppg = X[:, 0, :]
    ecg = X[:, 1, :]

    cond1 = np.isnan(ppg).any(axis=1)
    cond2 = np.isnan(ecg).any(axis=1)
    ...
    # Y[:, 1] = (dbp_boxcox * 1000 - 573) * 100  # 放大误差

    return lmbda


def post_process(data_path, configs):
    # lmbda = _prepare_data(data_path)
    ...
    y_train[:, 1] = inv_boxcox((y_train[:, 1] / 100 + configs["bias"]) / 1000, configs["lmbda"])
    y_true[:, 1] = inv_boxcox((y_true[:, 1] / 100 + configs["bias"]) / 1000, configs["lmbda"])
    y_hat[:, 1] = inv_boxcox((y_hat[:, 1] / 100 + configs["bias"]) / 1000, configs["lmbda"])
