...
class LKA(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.conv1 = CBR(in_channel, out_channel, 5, stride, 2, groups=in_channel)
        self.conv_spatial = CBR(out_channel, out_channel, 7, 1, 9, groups=in_channel, dilation=3)
        self.conv2 = CBR(out_channel, out_channel, 1, 1, 0)
...
class LKABlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.spatial_attn = LKA(in_channel, out_channel, stride)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, ratio=16):
        ...
        self.residual = LKA(in_channels, out_channels, stride=stride)
...
class BPNet(nn.Module):
    def __init__(self, model_width, layer_dims, ratio):
        ...
        self.top_layer = nn.Conv1d(model_width * 16, model_width * 2, 1, 1, 0, bias=False)
        self.latlayer_1 = nn.Conv1d(model_width * 8, model_width * 2, 1, 1, 0, bias=False)
        self.latlayer_2 = nn.Conv1d(model_width * 4, model_width * 2, 1, 1, 0, bias=False)
        self.latlayer_3 = nn.Conv1d(model_width * 2, model_width * 2, 1, 1, 0, bias=False)

        self.share = CBR(model_width * 2, model_width * 2, 3, 1, 1)

        # SBP branch
        self.cbr_1 = nn.Sequential(
            CBR(model_width * 2, model_width * 4, 3, 2, 1),
            CBR(model_width * 4, model_width * 4, 3, 1, 1)
        )
        self.mlp_1 = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Flatten(),
            nn.Linear(model_width * 4, 1)
        )
    ...
    def forward(self, ppg_x, ecg_x):
        ...
        ecg_c2, ecg_c3, ecg_c4, ecg_c5 = self.ecg_net(ecg_x)

        c2 = torch.cat((ppg_c2, ecg_c2), dim=1)
        c3 = torch.cat((ppg_c3, ecg_c3), dim=1)
        c4 = torch.cat((ppg_c4, ecg_c4), dim=1)
        c5 = torch.cat((ppg_c5, ecg_c5), dim=1)

        p5 = self.top_layer(c5)
        p4 = self._upsmaple_add(p5, self.latlayer_1(c4))
        p3 = self._upsmaple_add(p4, self.latlayer_2(c3))
        p2 = self._upsmaple_add(p3, self.latlayer_3(c2))
