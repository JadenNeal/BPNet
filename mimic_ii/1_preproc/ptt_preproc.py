...
def create_norm_sub(start, end, mu, sigma):
    ...
    dset = h5py.File(os.path.join(data_path, "bp_dataset_ii.h5"))
    ...
    for i in range(start, end + 1):
        ...
        cond = np.where((all_dbp >= i) & (all_dbp < i + 1))  # 处于(i, i+1)区间内的所有样本
        aim_bp = all_bp[cond]
        aim_wav = all_wav[cond]
...
def norm_pdf(x, mu, sigma): 
    # 计算概率密度
    pdf = 1/(sigma*np.sqrt(2*np.pi))*np.exp(-(x-mu)**2/(2*sigma**2)) * 200000
    # 75000 = 3000 / 0.04
    return pdf
...
def create_h5_new():
    ...
    f = h5py.File(os.path.join(path, "bp_dataset_norm.h5"), "w")  # append mode
