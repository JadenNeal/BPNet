...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, ratio=16):
        ...
        self.squeeze = nn.AdaptiveAvgPool1d(1)  # (N, C, L) -> (N, C, 1)
    ...
    def forward(self, inputs):
        identity = self.downsample(inputs)  # short cut
...
class BPNet(nn.Module):
    def __init__(self, model_width, layer_dims, ratio):
        ...
        self.cbr_1 = nn.Sequential(
            nn.Conv1d(model_width * 2, model_width * 4, 3, 2, 1),
            nn.BatchNorm1d(model_width * 4),
            nn.ReLU(inplace=True),
            nn.Conv1d(model_width * 4, model_width * 4, 3, 1, 1),
            nn.BatchNorm1d(model_width * 4),
            nn.ReLU(inplace=True)
        )
