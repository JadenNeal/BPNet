...
def compute_metrics(y_hat, y_true, y_train, save_path, configs):
    ...
    grad_a = np.sum(abs_error <= 5, axis=0) / length * 100
    grad_b = np.sum(abs_error <= 10, axis=0) / length * 100
    grad_c = np.sum(abs_error <= 15, axis=0) / length * 100

    me = np.sum((y_hat - y_true), axis=0) / length
    mae = np.sum(abs_error, axis=0) / length
    std = np.std(abs_error, axis=0, ddof=1)
    ...
    with open(os.path.join(save_path, 'bp_metric.txt'), "w+") as f:
        ...
        f.write("BHS Grade: \n")
        f.write(f"Grade A: {grad_a[0]:.2f}\t")
        f.write(f"Grade B: {grad_b[0]:.2f}\t")
        f.write(f"Grade C: {grad_c[0]:.2f}\n")
        ...
        f.write("BHS Grade: \n")
        f.write(f"Grade A: {grad_a[1]:.2f}\t")
        f.write(f"Grade B: {grad_b[1]:.2f}\t")
        f.write(f"Grade C: {grad_c[1]:.2f}\n")
...
if __name__ == "__main__":
    d_path = r"C:\Users\ranfe\Downloads\test"
    configs = {
        "epochs": 100,
        "batch_size": 64,
        "model_width": 64,
