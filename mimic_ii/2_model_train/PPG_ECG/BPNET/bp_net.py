...
class Attention(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.proj_1 = nn.Conv1d(in_channel, out_channel, 3, 2, 1)
        self.proj_2 = nn.Conv1d(out_channel, out_channel, 1, 1, 0)
        self.bn = nn.BatchNorm1d(out_channel)
...
class BPNet(nn.Module):
    def __init__(self, model_width, layer_dims, ratio):
        ...
        self.attn_1 = Attention(model_width * 2, model_width * 4)
        self.attn_2 = Attention(model_width * 4, model_width * 8)
        self.attn_3 = Attention(model_width * 8, model_width * 16)

        self.share = CBR(model_width * 16, model_width * 16, 3, 1, 1)

        # SBP branch
        self.sbp = nn.Sequential(
            CBR(model_width * 16, model_width * 8, 3, 2, 1),
            CBR(model_width * 8, model_width * 8, 3, 1, 1),
    ...
    def forward(self, ppg_x, ecg_x):
        ...
        c2 = torch.cat((ppg_c2, ecg_c2), dim=1)  # (N, mw * 2, L)
        c3 = torch.cat((ppg_c3, ecg_c3), dim=1)  # (N, MW * 4, L / 2)
        c4 = torch.cat((ppg_c4, ecg_c4), dim=1)
        c5 = torch.cat((ppg_c5, ecg_c5), dim=1)

        p3 = self.attn_1(c2, c3)
        p4 = self.attn_2(p3, c4)
        p5 = self.attn_3(p4, c5)

        share_x = self.share(p5)

        sbp = self.sbp(share_x)
        dbp = self.dbp(share_x)
