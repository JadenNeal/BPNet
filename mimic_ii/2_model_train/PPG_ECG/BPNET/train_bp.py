...
def _prepare_data(path):
    ...
    dbp_boxcox, lmbda = boxcox(Y[:, 1])
    Y[:, 1] = (dbp_boxcox * 1000 - configs["bias"]) * 100  # 放大误差

    x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=configs["test_size"], shuffle=True, random_state=0)
    print("load success ...")

    with open("lambda.txt", "w+") as f:
        f.write(f"boxcox lambda: {lmbda}")
...
def main():
    # 全部数据
    root_path = r"/data_new/lwc/mimic_ii/dataset_final"
